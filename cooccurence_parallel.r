...
# Load tree and data
tree <- read.newick("/share/huerta1/pjoglek/statitical/core_gene_alignment_filtered_align2.newick")
data <- read.csv("/Users/pjoglekar/statistical/Converged_presence_absence_2.txt", row.names = 1, sep = "\t", header = TRUE, check.names = FALSE)
...
for (i in 1:(length(systems) - 1)) {
    for (j in (i + 1):length(systems)) {
        ...
        fit_indep <- fitDiscrete(tree_td, cbind(A, B), model = "ARD", ncores = 14)
